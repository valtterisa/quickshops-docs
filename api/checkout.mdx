...
description: "POST /v1/checkout/session — creates a Stripe-hosted checkout session from a cart ID and returns a single-use redirect URL."
---

The Checkout endpoint converts a cart into a Stripe-hosted checkout session. You call this endpoint from your server, receive a URL, and redirect the customer's browser to that URL to complete payment.
